<template>
  <v-sheet class="ma-8  d-flex gr-8 flex-column" max-width="720">
    <v-sheet class="d-flex justify-space-between" max-width="350px">
      ...
      <v-sheet class="d-flex flex-column gr-5" v-if="type">
        <h5
          class="text-body-1 font-weight-bold text-uppercase"
          style="line-height: 24px; color: black"
        >
        ...
        <div style="font-weight: 600">
          <span class="d-flex gc-2">
            <v-icon icon="$tb-user-circle" size="24" />
            <p>{{ caseDetails?.partnerBookingVet ?? ' -' }}</p>
          </span>
        </div>
      </v-sheet>
    </v-sheet>
    ...
    <v-sheet class="d-flex flex-column gr-5">
      ...
      <div style="font-weight: 600; max-width: 365px" class="d-flex justify-space-between">
        <span class="d-flex gc-2">
          <v-icon icon="$tb-user-circle" size="24" />
      ...
      <v-sheet max-width="580" class="w-100 d-flex justify-space-between">
        <div class="w-100 pet-details--grid">
          <div class="d-flex flex-column" style="gap: 11px; font-weight: 600">
            ...
            <p class="text-grey2 font-weight-bold">
              Gender:
              <span style="color: black">{{ caseDetails?.pet.gender ?? '-' }}</span>
            </p>
            <p class="text-grey2 font-weight-bold">
          ...
          <div class="d-flex flex-column" style="gap: 11px; font-weight: 600">
            <p class="text-grey2 font-weight-bold">
              Weight:
              <span style="color: black">{{ caseDetails?.pet.weight ?? ' -' }} kg</span>
            </p>
            <p class="text-grey2 font-weight-bold">
              Chip Number:
    ...
    <v-sheet class="d-flex flex-column gr-6">
      ...
      <v-sheet class="d-flex flex-column gr-6">
        <HealthHistoryRow
          v-if="caseDetails?.pet.healthHistory.length"
          v-for="condition in caseDetails?.pet.healthHistory"
          :key="condition.id"
          :condition="condition"
        />
        <p v-else>-</p>
