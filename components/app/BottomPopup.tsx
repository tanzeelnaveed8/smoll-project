...
interface BottomPopupProps {
  type: "emergencyCase" | "appointment" | "quotation"
  petName:string
}
...
const popUpTypes = {
  ...
  quotation:{
    ...
    description: (petName:string) =>  `Quotation for ${petName}'s\ncase has been recived,\nlet's take a look at it.`,
...
const BottomPopup: React.FC<BottomPopupProps> = ({type , petName}) => {
      ...
      <Div flex={1} row={true} position="relative">
        <Div flex={1} row={false} justifyContent="space-between" h={"100%"} w={"100%"} mr={60} >
          ...
          <ButtonPrimary py={6} maxW={210} bg="#fff" fontWeight="500">
            ...
            <Text mr={8} fontSize={18} color={popUpTypes[type].bgColor} fontFamily={fontHauoraBold}>
             {popUpTypes[type].buttonText}
            </Text>
            <IconArrowRight width={34} height={34} strokeWidth={2.7} color={popUpTypes[type].bgColor} />
            </Div>
          </ButtonPrimary>
        </Div>
