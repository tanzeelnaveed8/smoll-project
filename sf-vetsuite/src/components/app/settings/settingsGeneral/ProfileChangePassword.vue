<template>
  <v-sheet class="w-100 pa-8 d-flex flex-column gr-6">
...
const handleFormSubmit = async () => {
  ...
  try {
    await updateUser({ oldPassword: account.value.oldPwd, password: account.value.newPwd })
...
watch(
  () => account,
  (v) => {
    if (!v.value.oldPwd?.length || !v.value.newPwd?.length || !v.value.repeatPwd?.length) {
      disableSubmitBtn.value = true
    } else {
