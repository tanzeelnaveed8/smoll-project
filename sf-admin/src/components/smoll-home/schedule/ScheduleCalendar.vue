<template>
  <v-card rounded="lg" elevation="0" border>
    <v-card-text class="pa-3 pa-sm-4">
      ...
      <template v-else>
        ...
        <div class="calendar-grid calendar-body">
          ...
          >
            ...
            <div v-for="event in cell.events.slice(0, 2)" :key="event.id" class="mt-1">
              <v-chip
                size="x-small"
                :color="getStatusColor(event.status)"
                variant="tonal"
                class="w-100 justify-start event-chip"
                :to="`/smoll-home/visits/${event.consultationId}`"
              >
      ...
      <div v-if="upcomingEvents.length" class="mt-4">
          ...
          >
            <div class="d-flex justify-space-between align-center">
              <div style="min-width: 0">
                <p class="text-body-2 font-weight-medium text-truncate">{{ event.title || 'Visit' }}</p>
                <p class="text-caption text-grey-darken-1">{{ formatDateTime(event.start) }}</p>
              </div>
              <v-chip :color="getStatusColor(event.status)" size="x-small" variant="tonal" class="ml-2 flex-shrink-0">
...
import type { CalendarEvent } from '@/stores/types/vet-types'
import { formatDateTime, formatTime, getStatusColor, getStatusLabel } from '@/util/vet-helpers'
...
const events = computed(() => visitsStore.calendarEvents)
...
const calendarCells = computed((): CalendarCell[] => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const dayEvents = events.value.filter((e) => dayjs(e.start).format('YYYY-MM-DD') === dateStr)
...
const fetchCalendar = async () => {
  loading.value = true
  try {
    const startDate = currentDate.value.startOf('month').format('YYYY-MM-DDTHH:mm:ss') + 'Z'
    await visitsStore.fetchCalendar(startDate, 'monthly')
  } finally {
    loading.value = false
  }
}
