...
const emit = defineEmits<{
  (events: 'updateDetails', details: {}): void
}>()
...
watchEffect(() => {
  ...
  if (formDetails) {
    ...
    const country = formDetails?.country
      ? formDetails.country.charAt(0).toUpperCase() + formDetails.country.slice(1)
      : ''
...
watch(
  ...
  (v) => {
    ...
    const { profileImg, documents, ...rest } = v
